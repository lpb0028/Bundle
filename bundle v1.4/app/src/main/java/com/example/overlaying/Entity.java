...
public class Entity extends androidx.appcompat.widget.AppCompatImageView {
    ...
    private double jumpAnimPosition = 0;
    private double hurtAnimPosition = 0;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(event.getAction() == MotionEvent.ACTION_DOWN && !isInvulnerable) {
            ...
            // Begin hurt knockback animation
            isInvulnerable = true;
            groundState = -2;
            // Store modified position for use later
            if(jumpAnimPosition > 0)
                jumpAnimPosition = jumpAnimPosition - Settings.CELL_SIZE;

            //if(Settings.SEND_DEBUG_MESSAGES) System.out.println("HurtAnimPosition: " + hurtAnimPosition + ", JumpAnimPosition " + jumpAnimPosition);
        }
    ...
    public void doFrame() {
        ...
        // If touching ground and not falling, reset vertical position animation
        if (currentPosition.y % Settings.CELL_SIZE == 0 && groundState != -2) {
            groundState = 0;
            jumpAnimPosition = 0;
            hurtAnimPosition = 0;
        ...
        // If IDLE, state duration is up, able to move
        ...
        if (moveState == MoveState.MOVE || moveState == MoveState.EXIT) {
            ...
            // If not grounded and currently falling down
            if (groundState == 1) {
                // If animation completed, error correct and finish
                if (jumpAnimTimer >= Settings.JUMP_ANIM_DURATION)
                    movement.y = Math.max(Settings.CELL_SIZE, currentPosition.y % Settings.CELL_SIZE) - Math.min(Settings.CELL_SIZE, currentPosition.y % Settings.CELL_SIZE);
                // Otherwise continue animation
                else
                    movement.y = (float) AnimationCurve.Evaluate(AnimationCurve.AnimForm.CUBE, jumpAnimTimer / Settings.JUMP_ANIM_DURATION) * Settings.CELL_SIZE - currentPosition.y % Settings.CELL_SIZE;
                jumpAnimTimer += deltaTime;

            }
            // If not grounded and currently jumping up
            if (groundState == -1) {
                // If animation completed, error correct and finish
                if (jumpAnimTimer >= Settings.JUMP_ANIM_DURATION)
                    movement.y = -(Settings.CELL_SIZE + jumpAnimPosition);
                // Otherwise continue animation
                else {
                    float eval = -(float) AnimationCurve.Evaluate(AnimationCurve.AnimForm.CUSTOM1, jumpAnimTimer / Settings.JUMP_ANIM_DURATION) * Settings.CELL_SIZE;
                    movement.y = eval - jumpAnimPosition;
                }
                jumpAnimTimer += deltaTime;
            }
        }
        // If currently in hit recoil (grounded or not)
        if (groundState == -2) {
            // If animation done, finish and allow for next hit
            if (hurtAnimTimer >= Settings.HURT_ANIM_DURATION && hurtAnimPosition > 0) {
                movement.y = -hurtAnimPosition;
                groundState = 1;
                isInvulnerable = false;
            }
            // Otherwise continue hurt animation
            else {
                float eval = -(float) AnimationCurve.Evaluate(AnimationCurve.AnimForm.CUSTOM2, hurtAnimTimer / Settings.HURT_ANIM_DURATION) * Settings.CELL_SIZE;
                movement.y = eval - hurtAnimPosition;
                if(Settings.SEND_DEBUG_MESSAGES && groundState == -2) {
                    //System.out.println("ID:" + entityID + " Y movement this frame: " + movement.y + ", eval " + eval);
                }
                hurtAnimPosition += movement.y;
            }
            hurtAnimTimer += deltaTime;
        }
        // If movement, update position, otherwise no need to
        if (!movement.equals(new Vector2())) {
            setPosition(currentPosition.plus(movement));
            invalidate();
        }
        // If jumping or falling, update associated position var
